...
public class SmartThermostatServlet extends HttpServlet {
    @Override
    public void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            // Fetch Smart Thermostat products from the database
            ResultSet rs = MysqlDataStoreUtilities.getSmartThermostatProducts();

            pw.println("<html><head>");
            pw.println("<title>Smart Thermostat Catalog</title>");
            pw.println("<link rel='stylesheet' type='text/css' href='style.css'>");
            pw.println("</head><body>");
            pw.println("<div id='content' class='postcard-container'>");

            // Iterate through the ResultSet and display smart thermostat products
            while (rs != null && rs.next()) {
                ...
                String productCategory = "SmartThermostat"; // Set the product category to Speaker/
 pw.println("<button class='add-review' onclick=\"showReviewForm('" + name + "', '" + price + "', '" + manufacturer + "', '" + productCategory + "')\">Add Review</button>");
            ...
            pw.println("<p>Error displaying smart thermostat products: " + e.getMessage() + "</p>");
            pw.println("</body></html>");
        } finally {
            pw.close();
        }
